...
protected:
    void SetUp() override {
        ...
        // legacy registry wiring removed; keep plugin construction direct
...
std::unordered_map<std::string, std::shared_ptr<IPluggableManager>> makeTestPluginMap(
    ...
    plugins["bdt"] = std::shared_ptr<IPluggableManager>(std::move(bdt));
    plugins["correction"] = std::shared_ptr<IPluggableManager>(std::move(corr));
    plugins["trigger"] = std::shared_ptr<IPluggableManager>(std::move(trig));
    //plugins["ndhist"] = std::shared_ptr<IPluggableManager>(dynamic_cast<IPluggableManager*>(ndh.release()));
    return plugins;
}
