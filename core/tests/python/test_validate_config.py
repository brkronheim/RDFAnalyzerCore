...
- validate_submit_config routing to YAML validation when sampleConfig is a
  YAML file
- _parse_opendata_config (in law/opendata_tasks) for both legacy
...
class TestLoadSampleConfigLegacyCompatibility:
    """Verify the explicit legacy .txt compatibility adapter behavior."""

    def test_load_sample_config_txt_emits_deprecation_warning(self, tmp_path):
        from rucio_discovery import load_sample_config

        sample_txt = _write(tmp_path, "samples.txt", """\
            name=ttbar das=/TTto2L2Nu/Run3/NANO xsec=98.34 type=0 sum_weights=100000.0
        """)
        with pytest.warns(DeprecationWarning, match="Legacy text sample configs are deprecated"):
            data = load_sample_config(sample_txt)

        assert data.entries["ttbar"].name == "ttbar"
        assert data.entries["ttbar"].das == "/TTto2L2Nu/Run3/NANO"
...
class TestParseOpendataConfig:
    ...
    def test_legacy_text_format_rejected(self, tmp_path):
        path = _write(tmp_path, "samples.txt", """\
            lumi=59.7
            recids=12345,67890
            name=mydata das=CMS_Run2012B xsec=1.0 type=1
        """)
        with pytest.raises(ValueError, match="Open Data sample manifests must be YAML"):
            _parse_opendata_config(str(path))
    ...
    def test_yaml_manifest_format(self, tmp_path):
        path = _write(tmp_path, "samples.yaml", """\
            ...
            datasets:
              - name: mydata
                ...
                das: "54321"
                files:
                  - root://eos.example//store/data/file1.root
              - name: mymc
                dtype: mc
                das: /TTto2L2Nu/Run3/NANO
                xsec: 98.34
        ...
        assert samples["mydata"].files == ["root://eos.example//store/data/file1.root"]
        # Explicit file lists bypass record lookup collection
        assert "54321" not in recids
