#!/usr/bin/env python3
"""
Tests for the rucio_tasks Law module.

These tests verify the XRootD redirector utility is correctly enforced on
all Rucio-discovered dataset file URIs, without requiring a running Law
scheduler, an HTCondor cluster, Rucio, or a VOMS proxy.
"""
...
@unittest.skipUnless(_LAW_AVAILABLE, _SKIP_MSG)
class TestRucioTasksCompatibilityBoundary(unittest.TestCase):
    """Ensure active LAW workflow imports only the YAML-first sample-config boundary."""

    def test_rucio_tasks_only_exports_load_sample_config(self):
        import rucio_tasks

        self.assertTrue(
            hasattr(rucio_tasks, "load_sample_config"),
            "Active Rucio workflow modules must import the YAML-first loader",
        )
        self.assertFalse(
            hasattr(rucio_tasks, "load_legacy_sample_config"),
            "Legacy .txt compatibility loader must not be exposed as an active workflow symbol",
        )
