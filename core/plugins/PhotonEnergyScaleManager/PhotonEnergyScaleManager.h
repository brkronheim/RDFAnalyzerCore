...
/**
 * @brief Record of a single photon energy systematic variation.
 */
 ...
 * @brief Plugin for applying CMS Photon Energy Scale (PES) and Resolution
 *        (EER) corrections to photon collections, with full support for
 ...
 *
 * ## Features
 *  - **Correctionlib-based scale and smearing**: apply PES/EER corrections
 *    to a per-photon pT column via CorrectionManager.
 *  - **Systematic variations**: register named up/down variations with
 *    ISystematicManager.
 *  - **PhysicsObjectCollection integration**: consume a
 *    PhysicsObjectCollection of photons and produce corrected nominal and
 ...
 *
 *   // 1. Declare photon column names.
 *   ees->setObjectColumns("Photon_pt", "Photon_eta",
 *                         "Photon_phi", "Photon_mass");
 *
 *   // 2. Apply nominal PES via correctionlib.
 *   ees->applyCorrectionlib(*cm, "photonScale", {"nom"},
 ...
 *                                             "scaleSystUp",
 *                                             "scaleSystDown"});
 *   ees->applySystematicSet(*cm, "photonScale", "escale",
 *                           "Photon_pt", "Photon_pt_ees");
 *
 *   // 4. PhysicsObjectCollection integration.
 *   ees->setInputCollection("goodPhotons");
 *   ees->defineCollectionOutput("Photon_pt_escale", "goodPhotons_corr");
...
public:
  ...
  /**
   * @brief Schedule a correction using an existing per-photon SF column.
   *
   * In execute():
   * @code
