...
std::string ObjectEnergyManagerBase::toLower(const std::string &s) {
  std::string r = s;
...
std::string ObjectEnergyManagerBase::deriveMassColumnName(
...
void ObjectEnergyManagerBase::setContext(ManagerContext &ctx) {
...
void ObjectEnergyManagerBase::setMETColumns(const std::string &metPtColumn,
                                             const std::string &metPhiColumn) {
  if (metPtColumn.empty())
    throw std::invalid_argument(
        type() + "::setMETColumns: metPtColumn must not be empty");
...
void ObjectEnergyManagerBase::applyCorrection(
  ...
  if (inputPtColumn.empty())
    throw std::invalid_argument(
        type() + "::applyCorrection: inputPtColumn must not be empty");
  if (sfColumn.empty())
    throw std::invalid_argument(
        type() + "::applyCorrection: sfColumn must not be empty");
  if (outputPtColumn.empty())
    throw std::invalid_argument(
        type() + "::applyCorrection: outputPtColumn must not be empty");
...
void ObjectEnergyManagerBase::applyResolutionSmearing(
    const std::string &inputPtColumn, const std::string &sigmaColumn,
    const std::string &randomColumn, const std::string &outputPtColumn) {
  if (inputPtColumn.empty())
    throw std::invalid_argument(
        type() + "::applyResolutionSmearing: inputPtColumn must not be empty");
  if (sigmaColumn.empty())
    throw std::invalid_argument(
        type() + "::applyResolutionSmearing: sigmaColumn must not be empty");
  if (randomColumn.empty())
    throw std::invalid_argument(
        type() + "::applyResolutionSmearing: randomColumn must not be empty");
  if (outputPtColumn.empty())
    throw std::invalid_argument(
        type() + "::applyResolutionSmearing: outputPtColumn must not be empty");
...
void ObjectEnergyManagerBase::registerSystematicSources(
    const std::string &setName, const std::vector<std::string> &sources) {
  if (setName.empty())
    throw std::invalid_argument(
        type() + "::registerSystematicSources: setName must not be empty");
  if (sources.empty())
    throw std::invalid_argument(
        type() + "::registerSystematicSources: sources must not be empty");
...
ObjectEnergyManagerBase::getSystematicSources(
    const std::string &setName) const {
  auto it = systematicSets_m.find(setName);
  if (it == systematicSets_m.end())
    throw std::out_of_range(
        type() + "::getSystematicSources: set \"" + setName +
        "\" is not registered");
  return it->second;
}

void ObjectEnergyManagerBase::applySystematicSet(
    CorrectionManager &cm, const std::string &correctionName,
  ...
  if (correctionName.empty())
    throw std::invalid_argument(
        type() + "::applySystematicSet: correctionName must not be empty");
  if (inputPtColumn.empty())
    throw std::invalid_argument(
        type() + "::applySystematicSet: inputPtColumn must not be empty");
  if (outputPtPrefix.empty())
    throw std::invalid_argument(
        type() + "::applySystematicSet: outputPtPrefix must not be empty");
...
void ObjectEnergyManagerBase::addVariation(
  ...
  if (systematicName.empty())
    throw std::invalid_argument(
        type() + "::addVariation: systematicName must not be empty");
  if (upPtColumn.empty())
    throw std::invalid_argument(
        type() + "::addVariation: upPtColumn must not be empty");
  if (downPtColumn.empty())
    throw std::invalid_argument(
        type() + "::addVariation: downPtColumn must not be empty");

  EnergyVariationEntry entry;
...
void ObjectEnergyManagerBase::propagateMET(
  ...
  if (baseMETPtColumn.empty())
    throw std::invalid_argument(
        type() + "::propagateMET: baseMETPtColumn must not be empty");
  if (baseMETPhiColumn.empty())
    throw std::invalid_argument(
        type() + "::propagateMET: baseMETPhiColumn must not be empty");
  if (nominalPtColumn.empty())
    throw std::invalid_argument(
        type() + "::propagateMET: nominalPtColumn must not be empty");
  if (variedPtColumn.empty())
    throw std::invalid_argument(
        type() + "::propagateMET: variedPtColumn must not be empty");
  if (outputMETPtColumn.empty())
    throw std::invalid_argument(
        type() + "::propagateMET: outputMETPtColumn must not be empty");
  if (outputMETPhiColumn.empty())
    throw std::invalid_argument(
        type() + "::propagateMET: outputMETPhiColumn must not be empty");
  if (phiColumn_m.empty())
    throw std::runtime_error(
        type() + "::propagateMET: object phi column not set. "
        "Call setObjectColumns() before propagateMET().");
...
void ObjectEnergyManagerBase::setInputCollection(
...
void ObjectEnergyManagerBase::defineCollectionOutput(
  ...
  if (correctedPtColumn.empty())
    throw std::invalid_argument(
        type() + "::defineCollectionOutput: correctedPtColumn must not be empty");
  if (outputCollectionColumn.empty())
    throw std::invalid_argument(
        type() + "::defineCollectionOutput: outputCollectionColumn must not be empty");
  if (inputCollectionColumn_m.empty())
    throw std::runtime_error(
        type() + "::defineCollectionOutput: "
...
void ObjectEnergyManagerBase::defineVariationCollections(
  ...
  if (nominalCollectionColumn.empty())
    throw std::invalid_argument(
        type() + "::defineVariationCollections: "
...
const std::string &ObjectEnergyManagerBase::getMETPtColumn() const {
  return metPtColumn_m;
}
const std::string &ObjectEnergyManagerBase::getMETPhiColumn() const {
  return metPhiColumn_m;
}
const std::string &ObjectEnergyManagerBase::getInputCollectionColumn() const {
  return inputCollectionColumn_m;
}
const std::vector<EnergyVariationEntry> &
ObjectEnergyManagerBase::getVariations() const {
  return variations_m;
}
...
void ObjectEnergyManagerBase::execute() {
  ...
  for (const auto &step : correctionSteps_m) {
    {
      ...
      const std::string inPt  = step.inputPtColumn;
      const std::string sf    = step.sfColumn;
      const std::string outPt = step.outputPtColumn;
    ...
    if (!step.inputMassColumn.empty() && !step.outputMassColumn.empty()) {
      ROOT::RDF::RNode df = dataManager_m->getDataFrame();
      const std::string inMass  = step.inputMassColumn;
      const std::string sf      = step.sfColumn;
      const std::string outMass = step.outputMassColumn;
      auto newDf = df.Define(
          outMass,
  ...
  // 2. Resolution smearing steps: output_pt = input_pt + sigma × u.
  ...
  //    An intermediate column {MET_x_new, MET_y_new} avoids computing the
  //    sum twice. The naming uses the output pT column to keep it unique.
  for (const auto &step : metPropagationSteps_m) {
    const std::string tmpCol =
        "_" + toLower(objectName()) + "met_tmp_" + step.outputMETPtColumn;
    ...
    // Intermediate: RVec<float>{new_MET_x, new_MET_y}.
    {
      ROOT::RDF::RNode df = dataManager_m->getDataFrame();
      auto newDf = df.Define(
          tmpCol,
  ...
  for (const auto &varColStep : variationCollectionsSteps_m) {
    ...
    // Build PhysicsObjectVariationMap if requested.
    if (!mapCol.empty()) {
      {
        ROOT::RDF::RNode df = dataManager_m->getDataFrame();
        auto newDf = df.Define(
...
void ObjectEnergyManagerBase::reportMetadata() {
  if (!logger_m)
    return;

  std::ostringstream ss;
  ss << type() << ": configuration summary\n";

  if (!ptColumn_m.empty())
    ss << "  " << objectName() << " columns:"
       << " pt="   << ptColumn_m
       << " eta="  << etaColumn_m
       << " phi="  << phiColumn_m
       << " mass=" << massColumn_m << "\n";
...
ObjectEnergyManagerBase::collectProvenanceEntries() const {
  std::unordered_map<std::string, std::string> entries;

  const std::string prefix = toLower(objectName());

  if (!ptColumn_m.empty()) {
    entries[prefix + "_pt_column"]   = ptColumn_m;
    entries[prefix + "_eta_column"]  = etaColumn_m;
    entries[prefix + "_phi_column"]  = phiColumn_m;
    entries[prefix + "_mass_column"] = massColumn_m;
  }
