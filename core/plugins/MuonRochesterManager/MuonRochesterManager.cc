...
void MuonEnergyScaleManager::setContext(ManagerContext &ctx) {
...
void MuonEnergyScaleManager::applyCorrection(
    const std::string &inputPtColumn, const std::string &sfColumn,
    const std::string &outputPtColumn) {
  if (inputPtColumn.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::applyCorrection: inputPtColumn must not be empty");
  if (sfColumn.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::applyCorrection: sfColumn must not be empty");
  if (outputPtColumn.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::applyCorrection: outputPtColumn must not be empty");
...
void MuonEnergyScaleManager::applyCorrectionlib(
...
void MuonEnergyScaleManager::registerSystematicSources(
    const std::string &setName, const std::vector<std::string> &sources) {
  if (setName.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::registerSystematicSources: setName must not be empty");
  if (sources.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::registerSystematicSources: sources must not be empty");
  for (const auto &s : sources) {
    if (s.empty())
      throw std::invalid_argument(
          "MuonEnergyScaleManager::registerSystematicSources: "
...
MuonEnergyScaleManager::getSystematicSources(
    const std::string &setName) const {
  auto it = systematicSets_m.find(setName);
  if (it == systematicSets_m.end())
    throw std::out_of_range(
        "MuonEnergyScaleManager::getSystematicSources: set \"" + setName +
        "\" is not registered");
  return it->second;
}

void MuonEnergyScaleManager::applySystematicSet(
    CorrectionManager &cm, const std::string &correctionName,
  ...
  if (correctionName.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::applySystematicSet: correctionName must not be empty");
  if (inputPtColumn.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::applySystematicSet: inputPtColumn must not be empty");
  if (outputPtPrefix.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::applySystematicSet: outputPtPrefix must not be empty");
...
void MuonEnergyScaleManager::addVariation(const std::string &systematicName,
                                               const std::string &upPtColumn,
                                               const std::string &downPtColumn) {
  if (systematicName.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::addVariation: systematicName must not be empty");
  if (upPtColumn.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::addVariation: upPtColumn must not be empty");
  if (downPtColumn.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::addVariation: downPtColumn must not be empty");

  RochesterVariationEntry entry;
...
void MuonEnergyScaleManager::defineVariationCollections(
  ...
  if (nominalCollectionColumn.empty())
    throw std::invalid_argument(
        "MuonEnergyScaleManager::defineVariationCollections: "
...
const std::string &MuonEnergyScaleManager::getPtColumn() const {
  return ptColumn_m;
}

const std::string &MuonEnergyScaleManager::getInputCollectionColumn() const {
  return inputCollectionColumn_m;
}

const std::vector<RochesterVariationEntry> &
MuonEnergyScaleManager::getVariations() const {
  return variations_m;
}
...
void MuonEnergyScaleManager::execute() {
  if (!dataManager_m)
    throw std::runtime_error("MuonEnergyScaleManager::execute: context not set");
...
void MuonEnergyScaleManager::reportMetadata() {
  if (!logger_m)
    return;

  std::ostringstream ss;
  ss << "MuonEnergyScaleManager: configuration summary\n";
...
MuonEnergyScaleManager::collectProvenanceEntries() const {
  std::unordered_map<std::string, std::string> entries;

  if (!ptColumn_m.empty()) {
    entries["muon_pt_column"] = ptColumn_m;
    entries["muon_eta_column"] = etaColumn_m;
    entries["muon_phi_column"] = phiColumn_m;
    entries["muon_mass_column"] = massColumn_m;
  }
