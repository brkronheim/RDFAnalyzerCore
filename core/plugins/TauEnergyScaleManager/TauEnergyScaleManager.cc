...
void TauEnergyScaleManager::registerSystematicSources(
  ...
  if (setName.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::registerSystematicSources: setName must not be empty");
  if (sources.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::registerSystematicSources: sources must not be empty");
...
void TauEnergyScaleManager::applySystematicSet(
  ...
  if (correctionName.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::applySystematicSet: correctionName must not be empty");
  if (inputPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::applySystematicSet: inputPtColumn must not be empty");
  if (outputPtPrefix.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::applySystematicSet: outputPtPrefix must not be empty");
...
void TauEnergyScaleManager::addVariation(const std::string &systematicName,
                                               const std::string &upPtColumn,
                                               const std::string &downPtColumn) {
  if (systematicName.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::addVariation: systematicName must not be empty");
  if (upPtColumn.empty())
    throw std::invalid_argument(
...
void TauEnergyScaleManager::execute() {
  if (!dataManager_m)
    throw std::runtime_error("TauEnergyScaleManager::execute: context not set");
