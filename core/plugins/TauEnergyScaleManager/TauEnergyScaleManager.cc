...
void TauEnergyScaleManager::setContext(ManagerContext &ctx) {
...
void TauEnergyScaleManager::setMETColumns(
    const std::string &metPtColumn, const std::string &metPhiColumn) {
  if (metPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::setMETColumns: metPtColumn must not be empty");
  if (metPhiColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::setMETColumns: metPhiColumn must not be empty");
...
std::string TauEnergyScaleManager::deriveMassColumnName(
...
void TauEnergyScaleManager::applyCorrection(
  ...
  if (inputPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::applyCorrection: inputPtColumn must not be empty");
  if (sfColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::applyCorrection: sfColumn must not be empty");
  if (outputPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::applyCorrection: outputPtColumn must not be empty");
...
void TauEnergyScaleManager::applyCorrectionlib(
...
void TauEnergyScaleManager::registerSystematicSources(
    const std::string &setName, const std::vector<std::string> &sources) {
  if (setName.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::registerSystematicSources: "
...
TauEnergyScaleManager::getSystematicSources(
    const std::string &setName) const {
  auto it = systematicSets_m.find(setName);
  if (it == systematicSets_m.end())
    throw std::out_of_range(
        "TauEnergyScaleManager::getSystematicSources: set \"" + setName +
        "\" is not registered");
  return it->second;
}

void TauEnergyScaleManager::applySystematicSet(
    CorrectionManager &cm, const std::string &correctionName,
  ...
  if (correctionName.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::applySystematicSet: "
...
void TauEnergyScaleManager::addVariation(
  ...
  if (systematicName.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::addVariation: "
        "systematicName must not be empty");
  if (upPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::addVariation: upPtColumn must not be empty");
  if (downPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::addVariation: downPtColumn must not be empty");

  TESVariationEntry entry;
...
void TauEnergyScaleManager::propagateMET(
  ...
  if (baseMETPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::propagateMET: "
...
void TauEnergyScaleManager::setInputCollection(
    const std::string &collectionColumn) {
  if (collectionColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::setInputCollection: "
        "collectionColumn must not be empty");
  inputCollectionColumn_m = collectionColumn;
}

void TauEnergyScaleManager::defineCollectionOutput(
  ...
  if (correctedPtColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::defineCollectionOutput: "
...
void TauEnergyScaleManager::defineVariationCollections(
  ...
  if (nominalCollectionColumn.empty())
    throw std::invalid_argument(
        "TauEnergyScaleManager::defineVariationCollections: "
...
const std::string &TauEnergyScaleManager::getPtColumn() const {
  return ptColumn_m;
}

const std::string &TauEnergyScaleManager::getMassColumn() const {
  return massColumn_m;
}

const std::string &TauEnergyScaleManager::getMETPtColumn() const {
  return metPtColumn_m;
}

const std::string &TauEnergyScaleManager::getMETPhiColumn() const {
  return metPhiColumn_m;
}

const std::string &TauEnergyScaleManager::getInputCollectionColumn() const {
  return inputCollectionColumn_m;
}

const std::vector<TESVariationEntry> &
TauEnergyScaleManager::getVariations() const {
  return variations_m;
}
...
void TauEnergyScaleManager::execute() {
  if (!dataManager_m)
    throw std::runtime_error(
        "TauEnergyScaleManager::execute: context not set");
  ...
  for (const auto &step : metPropagationSteps_m) {
    const std::string tmpCol =
        "_tesmet_tmp_" + step.outputMETPtColumn;
...
void TauEnergyScaleManager::reportMetadata() {
  if (!logger_m)
    return;

  std::ostringstream ss;
  ss << "TauEnergyScaleManager: configuration summary\n";
...
TauEnergyScaleManager::collectProvenanceEntries() const {
  std::unordered_map<std::string, std::string> entries;

  if (!ptColumn_m.empty()) {
    entries["tau_pt_column"]   = ptColumn_m;
    entries["tau_eta_column"]  = etaColumn_m;
    entries["tau_phi_column"]  = phiColumn_m;
    entries["tau_mass_column"] = massColumn_m;
  }
