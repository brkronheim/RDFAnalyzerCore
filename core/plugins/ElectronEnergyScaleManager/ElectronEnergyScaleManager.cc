...
void ElectronEnergyScaleManager::setMETColumns(
    const std::string &metPtColumn, const std::string &metPhiColumn) {
  if (metPtColumn.empty())
    throw std::invalid_argument(
        "ElectronEnergyScaleManager::setMETColumns: metPtColumn must not be empty");
  if (metPhiColumn.empty())
    throw std::invalid_argument(
        "ElectronEnergyScaleManager::setMETColumns: metPhiColumn must not be empty");
...
std::string ElectronEnergyScaleManager::deriveMassColumnName(
...
void ElectronEnergyScaleManager::applySystematicSet(
  ...
  if (correctionName.empty())
    throw std::invalid_argument(
        "ElectronEnergyScaleManager::applySystematicSet: "
...
void ElectronEnergyScaleManager::addVariation(
  ...
  if (systematicName.empty())
    throw std::invalid_argument(
        "ElectronEnergyScaleManager::addVariation: "
...
void ElectronEnergyScaleManager::propagateMET(
  ...
  if (baseMETPtColumn.empty())
    throw std::invalid_argument(
        "ElectronEnergyScaleManager::propagateMET: "
...
const std::string &ElectronEnergyScaleManager::getMassColumn() const {
  return massColumn_m;
}

const std::string &ElectronEnergyScaleManager::getMETPtColumn() const {
  return metPtColumn_m;
}

const std::string &ElectronEnergyScaleManager::getMETPhiColumn() const {
...
void ElectronEnergyScaleManager::execute() {
  if (!dataManager_m)
    throw std::runtime_error(
        "ElectronEnergyScaleManager::execute: context not set");
  ...
  for (const auto &step : metPropagationSteps_m) {
    const std::string tmpCol =
        "_eesmet_tmp_" + step.outputMETPtColumn;
