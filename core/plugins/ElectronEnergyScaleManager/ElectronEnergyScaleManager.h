...
/**
 * @brief Record of a single electron energy systematic variation.
 */
 ...
 * @brief Plugin for applying CMS Electron Energy Scale (EES) and Resolution
 *        (EER) corrections to electron collections, with full support for
 ...
 *
 * ## Features
 *  - **Correctionlib-based scale and smearing**: apply EES/EER corrections
 *    to a per-electron pT column via CorrectionManager.
 *  - **Systematic variations**: register named up/down variations with
 *    ISystematicManager.
 *  - **PhysicsObjectCollection integration**: consume a
 *    PhysicsObjectCollection of electrons and produce corrected nominal and
 ...
 *
 *   // 1. Declare electron column names.
 *   ees->setObjectColumns("Electron_pt", "Electron_eta",
 *                         "Electron_phi", "Electron_mass");
 *
 *   // 2. Apply nominal EES via correctionlib.
 *   ees->applyCorrectionlib(*cm, "electronScale", {"nom"},
 ...
 *                                             "scaleSystUp",
 *                                             "scaleSystDown"});
 *   ees->applySystematicSet(*cm, "electronScale", "escale",
 *                           "Electron_pt", "Electron_pt_ees");
 *
 *   // 4. PhysicsObjectCollection integration.
 *   ees->setInputCollection("goodElectrons");
 *   ees->defineCollectionOutput("Electron_pt_escale", "goodElectrons_corr");
...
public:
  ...
  /**
   * @brief Schedule a correction using an existing per-electron SF column.
   *
   * In execute():
   * @code
