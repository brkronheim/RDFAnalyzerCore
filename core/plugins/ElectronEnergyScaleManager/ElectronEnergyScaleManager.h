...
/**
 * @brief Record of a single electron energy scale/resolution systematic
 *        variation.
 */
struct EESVariationEntry {
 ...
 * @brief Plugin for applying CMS Electron Energy Scale (EES) and Energy
 *        Resolution (EER) corrections to electron collections, with full
 ...
 * @code
 *   auto* ees = analyzer->getPlugin<ElectronEnergyScaleManager>("ees");
 *   auto* cm  = analyzer->getPlugin<CorrectionManager>("corrections");
 *
 *   // 1. Declare electron and MET column names.
 *   ees->setObjectColumns("Electron_pt", "Electron_eta",
 *                         "Electron_phi", "Electron_mass");
 *   ees->setMETColumns("MET_pt", "MET_phi");
 *
 *   // 2. Apply nominal EES via correctionlib (multiplicative SF).
 *   ees->applyCorrectionlib(*cm, "electronSS", {"nom", "scale"},
 *                           "Electron_pt", "Electron_pt_escale");
 *
 *   // 3. Apply nominal EER resolution smearing.
 *   //    sigma column can come from correctionlib or be pre-defined.
 *   ees->applyResolutionSmearing("Electron_pt_escale",
 *                                "Electron_dEsigma",
 *                                "Electron_u1",
 *                                "Electron_pt_corr");
 *
 ...
 *   ees->applySystematicSet(*cm, "electronSS", "escale",
 *                           "Electron_pt", "Electron_pt_ees");
 *
 *   // 6. PhysicsObjectCollection integration.
 *   ees->setInputCollection("goodElectrons");
 *   ees->defineCollectionOutput("Electron_pt_corr", "goodElectrons_corr");
 *   ees->defineVariationCollections("goodElectrons_corr", "goodElectrons",
 *                                   "goodElectrons_variations");
 * @endcode
 */
class ElectronEnergyScaleManager : public IPluggableManager {
public:
  ElectronEnergyScaleManager() = default;
  ...
  /**
   * @brief Set the input electron kinematic column names.
   *
   * The phi column is also used as the source of electron directions for
   * MET propagation (electron φ does not change with EES/EER).
   *
   * @param ptColumn   Name of the per-electron pT column (RVec<Float_t>).
   * @param etaColumn  Name of the per-electron η column.
   * @param phiColumn  Name of the per-electron φ column.
   * @param massColumn Name of the per-electron mass column; empty = skip mass.
   *
   ...
   *
   * Typical CMS usage:
   *  - @p sigmaColumn   = per-electron resolution σ (from correctionlib or
   *    pre-computed, in GeV).
   *  - @p randomColumn  = per-electron Gaussian random number column
   ...
   * per-electron SF column, then calls applyCorrection().
   *
  ...
  /**
   * @brief Register a named list of EES/EER systematic source names.
   *
  ...
  /**
   * @brief Propagate an electron pT change into MET (Type-1 style).
   *
   * Computes:
   * @code
   ...
   * @endcode
   * where the sum runs over all electrons (no pT threshold by default since
   * electrons are already selected objects — pass @p ptThreshold > 0 to
   ...
   * @param nominalPtColumn     Nominal per-electron pT column (RVec<Float_t>).
   * @param variedPtColumn      Varied per-electron pT column (RVec<Float_t>).
   * @param outputMETPtColumn   Output scalar MET-pT column name.
   * @param outputMETPhiColumn  Output scalar MET-φ column name.
   * @param ptThreshold         Only propagate electrons with nominal pT above
   *                            this threshold (default: 0, i.e. all electrons).
   *
  ...
  /**
   * @brief Declare the RDF column holding the input electron
  ...
  const std::vector<EESVariationEntry> &getVariations() const;
...
private:
  ...
  std::vector<EESVariationEntry> variations_m;
